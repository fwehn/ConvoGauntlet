...
function read(data) {
    let currentGesture = data["f"].reduce((carry, value) => carry + readFlexSensor(value), "")
        + "_"
...
function onGesture(fn) {eventEmitter.on('gesture', fn);}

function onDebug(fn) {eventEmitter.on('debug', fn);}
